...
El principal desafío durante el desarrollo fue el \textbf{overfitting prematuro}, que se manifestó consistentemente en los primeros 15 experimentos. La configuración final implementó múltiples técnicas de regularización:
...
\hline
\rowcolor{LightGreen!30}
\textbf{Learning Rate (Tasa de aprendizaje) Ultra-Baja} & 2e-06 & Convergencia gradual & Reducción overfitting \\
\hline
\rowcolor{LightBlue!30}
...
\hline
\rowcolor{LightGoldenrod!30}
\textbf{Batch Size (Tamaño de lote) Pequeño} & 4 & Mayor ruido en gradientes & Regularización implícita \\
\hline
\rowcolor{LightPink!30}
...
\begin{itemize}
    ...
    \item \textbf{Batch sizes (número de ejemplos por lote) probados:} [4, 6, 8]
...
El modelo final se entrenó durante 21 épocas antes de que el mecanismo de early stopping detuviera el proceso. La \textbf{época 13 fue identificada como el punto óptimo}, marcando el momento antes del inicio del overfitting:
...
La métrica clave para evaluar el overfitting fue el \textbf{gap de pérdida} (diferencia entre pérdida de validación y entrenamiento):
...
\hline
\rowcolor{UAMPurple!20}
\textbf{Paradigma} & \textbf{Exactitud} & \textbf{Precisión} & \textbf{Recall} & \textbf{F1-Score} & \textbf{Especificidad} \\
\hline
\rowcolor{LightCoral!30}
...
\hline
\rowcolor{HeaderBlue!20}
\textbf{Mejora Absoluta} & \textbf{+24.1\%} & \textbf{+25.0\%} & \textbf{+21.5\%} & \textbf{+23.3\%} & \textbf{+26.8\%} \\
\hline
\end{tabular}
}
...
\textbf{Resultado principal:} El modelo DistilBERT fine-tuned supera consistentemente al mejor algoritmo metaheurístico en todas las métricas de evaluación, con mejoras absolutas que oscilan entre 21.5\% y 26.8\%.

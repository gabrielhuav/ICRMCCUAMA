...
El corpus de pre-entrenamiento de DistilBERT multilingüe incluye Wikipedia multilingüe con más de 100 idiomas representados, incluyendo español con aproximadamente 1.6 millones de artículos que representan más de 800 millones de palabras de texto enciclopédico de alta calidad. Este volumen de exposición permite al modelo aprender la morfología del español con patrones de conjugación verbal y concordancia de género y número, la sintaxis característica con orden flexible de palabras y construcciones con subjuntivo, la semántica contextual incluyendo polisemia y modismos específicos del español, y variación dialectal natural introducida por contribuidores de Wikipedia de diversos países hispanohablantes. El contenido total del corpus supera los 2.5 terabytes de texto limpio con cobertura balanceada que garantiza representación significativa de idiomas no anglosajones.

La metodología de pre-entrenamiento preservó la tarea de Masked Language Modeling (MLM) de BERT, donde el 15\% de los tokens son enmascarados y el modelo debe predecirlos basándose en el contexto bidireccional, pero eliminó Next Sentence Prediction (NSP) para simplificación y eficiencia. El proceso aplicó destilación de conocimiento triple mediante pérdida de destilación supervisando las salidas del modelo estudiante con las del profesor, pérdida de enmascaramiento de lenguaje manteniendo la tarea MLM original, y pérdida de similitud coseno alineando las representaciones ocultas entre estudiante y profesor. Esta triple destilación permite que DistilBERT con solo 6 capas Transformer (versus 12 en BERT) mantenga 97\% del rendimiento con 40\% menos parámetros, logrando un balance óptimo entre capacidad y eficiencia crucial para la fase intensiva de experimentación requerida en esta investigación.

Las características específicas para español en DistilBERT multilingüe incluyen vocabulario compartido mediante tokenizador WordPiece con 110,000 tokens compartidos entre todos los idiomas, proporcionando transferencia de conocimiento cross-lingual donde el aprendizaje en un idioma beneficia a otros debido a representaciones compartidas, y especialización implícita donde aunque no hay entrenamiento exclusivo en español, la arquitectura Transformer aprende patrones lingüísticos específicos mediante exposición a millones de oraciones en español durante el pre-entrenamiento. Contrario a la intuición inicial de que un modelo monolingüe especializado en español debería superar a uno multilingüe, la investigación reciente en PLN ha demostrado que los modelos multilingües ofrecen ventajas específicas mediante transferencia de conocimiento cross-lingual donde el modelo aprende representaciones universales que capturan patrones sintácticos y semánticos compartidos entre idiomas, robustez ante code-switching común en contenido digital hispanohablante donde se mezcla español e inglés, y generalización mejorada donde la exposición a múltiples idiomas durante el pre-entrenamiento actúa como regularización implícita.

Modelos alternativos considerados pero no seleccionados incluyen BETO (BERT en Español) pre-entrenado exclusivamente sobre 3 mil millones de palabras del corpus OPUS de traducciones y Wikipedia en español, ofreciendo especialización en español pero con limitación de tamaño del corpus comparado con DistilBERT multilingüe y mayor costo computacional al ser modelo completo no destilado; RoBERTa en español con arquitectura más robusta pero similar corpus ofreciendo rendimiento marginalmente superior pero sin justificar el incremento del 60\% en tiempo de entrenamiento y requerimientos de memoria; y mBERT (multilingual BERT) con 104 idiomas soportados pero siendo significativamente más grande (180M parámetros versus 134M de DistilBERT) resultando en menor eficiencia computacional sin mejoras proporcionales en rendimiento para la tarea específica de detección de noticias falsas. Experimentos preliminares con tres arquitecturas confirmaron empíricamente la selección: pruebas con TinyBERT en inglés (3 experimentos, 30-45 minutos cada uno) fueron descartadas por falta de soporte nativo para español requiriendo traducción que introduciría ruido adicional; pruebas con BERT completo (3 experimentos, 24-48 horas cada uno) alcanzaron exactitud de 94.8\% pero con tiempos de entrenamiento inviables para iteración experimental; y validación con DistilBERT (8+ experimentos, 12-24 horas cada uno) alcanzó exactitud final de 95.2\% con tiempos de experimentación 50\% menores que BERT completo.

La decisión final de utilizar DistilBERT multilingüe en lugar de variantes monolingües se fundamenta en disponibilidad y madurez siendo el modelo destilado multilingüe más estable y ampliamente validado, eficiencia comprobada manteniendo 97\% del rendimiento de BERT con 40\% menos parámetros validado empíricamente en múltiples benchmarks, soporte nativo para español sin necesidad de adaptación adicional o tokenizadores especializados, e integración con ecosistema mediante compatibilidad total con Hugging Face Transformers facilitando implementación. Esta selección optimiza el balance entre rendimiento (capacidad de detectar patrones complejos en español), eficiencia (recursos computacionales accesibles para entrenamiento), y generalización (robustez ante diferentes variantes del español multiregional), proporcionando la plataforma ideal para la investigación comparativa desarrollada en este proyecto.
